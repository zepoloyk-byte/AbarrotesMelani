...
// ====================================================================
const pb = new PocketBase('https://onyx-badger.pikapod.net');
...
// ====================================================================
function handleCompraScan(e, codigoDirecto = null) { 
    // Si viene código directo del buscador, o si el usuario presionó Enter físicamente...
    if(codigoDirecto || (e && e.key === 'Enter')) { 
        
        // Si hay código directo, usamos ese; si no, lo leemos del input tradicional
        let c = codigoDirecto ? codigoDirecto.trim() : document.getElementById('c_cod').value.trim(); 
        
        if(!c) return;

        // Forzamos a que el input visual muestre el código por si vino del buscador
        let inputCod = document.getElementById('c_cod');
        if(inputCod) inputCod.value = c;
        ...
        if(inv[c]) { 
            ...
            if (cReal > 0) {
                document.getElementById('c_gan').value = ((((inv[c].pv || 0) / cReal) - 1) * 100).toFixed(2);
            } else {
                document.getElementById('c_gan').value = inv[c].gan || 30;
            }

            // Movemos el foco a la cantidad para agilizar la carga
            setTimeout(() => { 
                let inputCant = document.getElementById('c_cant');
                if(inputCant) {
                    inputCant.focus(); 
                    inputCant.select(); 
                }
            ...
            setTimeout(() => { 
                let inputNom = document.getElementById('c_nom');
